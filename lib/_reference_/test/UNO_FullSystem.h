...
void loop() {
  ...
  #ifdef ARM_CONTROL
    ps2_arm.read_gamepad(false, vibrate_2);
  #endif // ARM_CONTROL
//
  ...
  //==========================================================
  #ifdef ARM_CONTROL
  if(ps2_arm.ButtonPressed(PSB_START)){
    Manipulator_Kinematic_Compute(RESET);
  ...
  // if(){  
    ...
    //=========================
    if(ps2_arm.ButtonPressed(PSB_L1)){ //gripper #1
      ArmMotor.MotorSpeed[GRIPPER_LEFT] = -QuarterSpeed;
    }
    if(ps2_arm.ButtonPressed(PSB_R1)){ //gripper #2
      ArmMotor.MotorSpeed[GRIPPER_RIGHT] = -QuarterSpeed*1.3;
    }
    if(ps2_arm.ButtonPressed(PSB_L2)){ //gripper #1
      ArmMotor.MotorSpeed[GRIPPER_LEFT] = QuarterSpeed;
    }
    if(ps2_arm.ButtonPressed(PSB_R2)){ //gripper #2
      ArmMotor.MotorSpeed[GRIPPER_RIGHT] = QuarterSpeed;
    }
    if(ps2_arm.ButtonReleased(PSB_L2)){ //gripper #1
      ArmMotor.MotorSpeed[GRIPPER_LEFT] = 0;
    }
    if(ps2_arm.ButtonReleased(PSB_R2)){ //gripper #2
      ArmMotor.MotorSpeed[GRIPPER_RIGHT] = 0;
    }
    if(ps2_arm.ButtonPressed(PSB_BLUE)){ // arm shoulder
      Manipulator_Kinematic_Compute(ARM_CYLO); // ARM_GRIPPING, ARM_CYLO, ARM_CART
      // Serial.println(ArmMotor.MotorPositions[2]);
    }
    if(ps2_arm.ButtonPressed(PSB_PINK)||ps2_arm.ButtonReleased(PSB_GREEN)||ps2_arm.ButtonReleased(PSB_BLUE)){ // arm shoulder
      Manipulator_Kinematic_Compute(ARM_GRIPPING); // ARM_GRIPPING, ARM_CYLO, ARM_CART
    }
    if(ps2_arm.ButtonPressed(PSB_GREEN)){ // arm shoulder
      Manipulator_Kinematic_Compute(ARM_CART); // ARM_GRIPPING, ARM_CYLO, ARM_CART
    }
    if(ps2_arm.ButtonPressed(PSB_RED)){ // magnet
      Manipulator_Kinematic_Compute(MAGNET_ON);
    }
    if(ps2_arm.ButtonPressed(PSB_SELECT)){
      DirSpeed = 255 * SpeedThreadhold;
      FreeRunning();
    }
    Execute_Arm_Gcode();
  // }
...
void PS2_Drivetrain(){
  ...
  ry= map(ps2_drivetrain.Analog(PSS_RY),-1,255, 255, -255);
  // Serial.print(ly);
  up = ps2_drivetrain.Button(PSB_PAD_UP);
  down = ps2_drivetrain.Button(PSB_PAD_DOWN);
  left = ps2_drivetrain.Button(PSB_PAD_LEFT);
  right = ps2_drivetrain.Button(PSB_PAD_RIGHT);
  if(up){
    DirSpeed = abs(255*SpeedThreadhold2);
    Execute_Drivetrain_Gcode(FORWARD);
  }
...
if (ly==0 && lx==0 && rx==0 && DirSpeed!= 0&& !up&& !down && !left && !right){
  DirSpeed = 0;
  Execute_Drivetrain_Gcode(STOP);
}
//==========================================================
...
if(ps2_drivetrain.ButtonPressed(PSB_SELECT)){
  freeRunning_robot.start(1000); // Start the timer
  rod.start(700);
  Manipulator_Kinematic_Compute(ROD_ON); // ARM_GRIPPING, ARM_CYLO, ARM_CART
  analogWrite(rodServo,ROD_ServoSpeed); // LED on

}
if(isButtonPressed_drivetrain()){  
  if(ps2_drivetrain.ButtonPressed(PSB_L1)){ //gripper #1
    closing_LeftGripper.start(700); // Start the timer
    ArmMotor.MotorSpeed[GRIPPER_LEFT] = OctorSpeed;
  }
  if(ps2_drivetrain.ButtonPressed(PSB_R1)){ //gripper #1
    closing_RightGripper.start(700); // Start the timer
    ArmMotor.MotorSpeed[GRIPPER_RIGHT] = OctorSpeed;
  }
  if(ps2_drivetrain.ButtonPressed(PSB_L2)){ //gripper #1
    closing_LeftGripper.start(180); // Start the timer
    ArmMotor.MotorSpeed[GRIPPER_LEFT] = -FullSpeed;
  ...
  if(ps2_drivetrain.ButtonReleased(PSB_GREEN)||ps2_drivetrain.ButtonReleased(PSB_BLUE)){ // arm shoulder
    Manipulator_Kinematic_Compute(ARM_GRIPPING); // ARM_GRIPPING, ARM_CYLO, ARM_CART
  }
  if(ps2_drivetrain.ButtonPressed(PSB_GREEN)){ // arm shoulder
    Manipulator_Kinematic_Compute(ARM_CART); // ARM_GRIPPING, ARM_CYLO, ARM_CART
  }
  if(ps2_drivetrain.ButtonPressed(PSB_RED)){ // magnet
    Manipulator_Kinematic_Compute(MAGNET_ON);
  }
  if(ps2_drivetrain.ButtonPressed(PSB_PINK)){
    Manipulator_Kinematic_Compute(ROD_ON); // ARM_GRIPPING, ARM_CYLO, ARM_CART
    
    ROD_ServoSpeed?rod.start(700):rod.start(1200); // Start the timer
    analogWrite(rodServo,ROD_ServoSpeed); // LED on
  }
  Execute_Arm_Gcode();
  }
  ...
  if (rod.stop()) { // Check if the timer has elapsed
    analogWrite(rodServo,LOW); // LED off
    Execute_Arm_Gcode();
  }
  if (freeRunning_robot.stop()) { // Check if the timer has elapsed
    FreeRunning();
  }
}
void PS2_Manipulator(){
  
  lx =map(ps2_arm.Analog(PSS_LX),0,256, -255, 255);
  ly =map(ps2_arm.Analog(PSS_LY),-1,255, 255, -255);
  rx= map(ps2_arm.Analog(PSS_RX),0,256, 255, -255);
  ry= map(ps2_arm.Analog(PSS_RY),-1,255, 255, -255);
  // Serial.print(ly);
  up = ps2_arm.ButtonPressed(PSB_PAD_UP);
  down = ps2_arm.ButtonPressed(PSB_PAD_DOWN);
  left = ps2_arm.ButtonPressed(PSB_PAD_LEFT);
  right = ps2_arm.ButtonPressed(PSB_PAD_RIGHT);
  if(up){
    DirSpeed = abs(255*SpeedThreadhold2);
    Execute_Drivetrain_Gcode(FORWARD);
  }
...
if(isButtonPressed_arm()){  
  if(ps2_arm.ButtonPressed(PSB_L1)){ //gripper #1
    closing_LeftGripper.start(700); // Start the timer
    ArmMotor.MotorSpeed[GRIPPER_LEFT] = OctorSpeed;
  }
  if(ps2_arm.ButtonPressed(PSB_R1)){ //gripper #1
    closing_RightGripper.start(700); // Start the timer
    ArmMotor.MotorSpeed[GRIPPER_RIGHT] = OctorSpeed;
  }
  if(ps2_arm.ButtonPressed(PSB_L2)){ //gripper #1
  ...
  if(ps2_arm.ButtonPressed(PSB_R2)){ //gripper #1
    closing_RightGripper.start(180); // Start the timer
    ArmMotor.MotorSpeed[GRIPPER_RIGHT] = -FullSpeed;
  }
  //=========================
  ...
  if(ps2_arm.ButtonReleased(PSB_GREEN)||ps2_arm.ButtonReleased(PSB_BLUE)){ // arm shoulder
    Manipulator_Kinematic_Compute(ARM_GRIPPING); // ARM_GRIPPING, ARM_CYLO, ARM_CART
  }
  if(ps2_arm.ButtonPressed(PSB_GREEN)){ // arm shoulder
    Manipulator_Kinematic_Compute(ARM_CART); // ARM_GRIPPING, ARM_CYLO, ARM_CART
  ...
  if(ps2_arm.ButtonPressed(PSB_PINK)){
    Manipulator_Kinematic_Compute(ROD_ON); // ARM_GRIPPING, ARM_CYLO, ARM_CART
    Serial.println(ROD_ServoSpeed);
    ROD_ServoSpeed?rod.start(700):rod.start(1200); // Start the timer
    analogWrite(rodServo,ROD_ServoSpeed); // LED on
  }
  Execute_Arm_Gcode();
